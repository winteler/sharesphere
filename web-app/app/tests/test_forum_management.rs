...
async fn test_ban_user_from_forum() -> Result<(), AppError> {
    ...
    let post = create_post(&forum.forum_name, "a", "b", None, false, false, None, false, &unauthorized_user, &db_pool).await?;
    ...
    assert!(create_post(&forum.forum_name, "c", "d", None, false, false, None, false, &unauthorized_user, &db_pool).await.is_err());
    ...
    assert!(create_post(&forum.forum_name, "c", "d", None, false, false, None, false, &banned_user, &db_pool).await.is_err());
